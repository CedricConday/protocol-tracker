...
export default function SettingsScreen() {
  ...
  const NavRow = ({ icon, label, sub, onPress }: { icon: string; label: string; sub?: string; onPress: () => void }) => (
    <TouchableOpacity style={styles.navRow} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); onPress(); }} activeOpacity={0.7}>
  ...
  const SectionHeader = ({ sectionKey, icon, label, sub }: { sectionKey: string; icon: string; label: string; sub?: string }) => (
    <TouchableOpacity style={styles.navRow} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); toggleSection(sectionKey); }} activeOpacity={0.7}>
      <Ionicons name={icon as any} size={20} color="#C96A50" style={{ marginRight: 12 }} />
      <View style={{ flex: 1 }}>
        <Text style={styles.navRowLabel}>{label}</Text>
        {sub ? <Text style={styles.navRowSub}>{sub}</Text> : null}
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
        {BEDTIME_HOURS.map((h) => (
          <TouchableOpacity
            ...
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setBedtimeHour(h); }}
            activeOpacity={0.7}
          >
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        {BEDTIME_MINUTES.map((m) => (
          <TouchableOpacity
            ...
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setBedtimeMinute(m); }}
            activeOpacity={0.7}
          >
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
            ...
            {expandedSection === 'profile' && (
              <View style={styles.expandedContent}>
                ...
                <View style={[styles.langRow, { marginBottom: 12 }]}>
                  {['en', 'de'].map((lang) => (
                    <TouchableOpacity key={lang} style={[styles.langButton, currentLanguage === lang ? styles.langActive : null]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); handleLanguageSwitch(lang); }} activeOpacity={0.7}>
                  ...
                  <View key={s.id} style={[styles.stockRow, idx === suppForms.length - 1 ? { borderBottomWidth: 0 } : null]}>
                    ...
                    <View style={styles.formRow}>
                      {(['capsule', 'tablet', 'powder', 'liquid'] as const).map((f) => (
                        <TouchableOpacity key={f} style={[styles.formBtn, s.form === f ? styles.formBtnActive : null]} onPress={async () => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); await updateSupplementForm(s.id, f); setSuppForms((prev) => prev.map((x) => x.id === s.id ? { ...x, form: f } : x)); }} activeOpacity={0.7}>
                    ...
                    return (
                      <View key={rule.id} style={[styles.toleranceRow, { borderBottomWidth: 0, marginBottom: 8 }]}>
                        ...
                        <View style={styles.stepper}>
                          <TouchableOpacity style={[styles.stepperBtn, val <= 15 ? styles.stepperBtnDisabled : null]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); handleToleranceChange(rule.id, -15); }} disabled={val <= 15} activeOpacity={0.6}><Text style={styles.stepperBtnText}>−</Text></TouchableOpacity>
                          <Text style={styles.stepperValue}>{val} min</Text>
                          <TouchableOpacity style={[styles.stepperBtn, val >= 120 ? styles.stepperBtnDisabled : null]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); handleToleranceChange(rule.id, 15); }} disabled={val >= 120} activeOpacity={0.6}><Text style={styles.stepperBtnText}>+</Text></TouchableOpacity>
                        </View>
                      </View>
                    );
                  })}
          ...
          <View style={styles.sectionCard}>
            ...
            <View style={styles.rowDivider} />
            <NavRow icon="mail-outline" label="Contact" sub="support@example.com" onPress={() => Alert.alert('Contact', 'Email us at support@example.com')} />
            <View style={styles.rowDivider} />
            <View style={[styles.navRow, { paddingVertical: 14 }]}>
          ...
          <View style={styles.dangerCard}>
            <View style={styles.dangerRow}>
              ...
              <TouchableOpacity style={styles.dangerBtn} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); handleResetAll(); }} activeOpacity={0.8}>
            ...
            <View style={[styles.dangerRow, { marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#2a2a2a' }]}>
              ...
              <TouchableOpacity style={[styles.dangerBtn, { borderColor: '#ef4444', backgroundColor: '#1a0a0a' }]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); handleDeleteAccount(); }} activeOpacity={0.8}>
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={[styles.button, saving ? styles.buttonDisabled : null]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); handleSave().then(() => Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)); }} disabled={saving}>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: 'transparent', borderRadius: 0, paddingHorizontal: 0, paddingVertical: 14, color: '#2C2420', fontSize: 16, borderBottomWidth: 1, borderBottomColor: '#D8CFC8', marginBottom: 2 },
  ...
  stockName: { color: '#7A6A62', fontSize: 15, fontWeight: '600', flex: 1 },
  ...
  toleranceName: { color: '#7A6A62', fontSize: 15, fontWeight: '600', flex: 1, marginRight: 12 },
  stepper: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  stepperBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: '#E8E0D8', alignItems: 'center', justifyContent: 'center' },
  ...
  privacyNotice: { color: '#B0A098', fontSize: 13, fontStyle: 'italic', marginTop: 8, marginBottom: 12, lineHeight: 20 },
  ...
  surveyRowTitle: { color: '#2C2420', fontSize: 15, fontWeight: '600' },
  surveyRowSub: { color: '#7A6A62', fontSize: 13, marginTop: 2 },
  ...
  formBtn: { width: 36, height: 36, borderRadius: 10, borderWidth: 1, borderColor: '#D8CFC8', alignItems: 'center', justifyContent: 'center' },
  ...
  nudgeBtn: { flex: 1, borderRadius: 10, borderWidth: 1, borderColor: '#D8CFC8', paddingVertical: 12, alignItems: 'center' },
  nudgeBtnActive: { borderColor: '#C96A50', backgroundColor: '#FBF0ED' },
  nudgeBtnText: { color: '#B0A098', fontSize: 13, fontWeight: '600' },
  nudgeBtnTextActive: { color: '#C96A50' },
  dangerCard: { backgroundColor: '#F2EDE8', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14, borderWidth: 1, borderColor: '#C0404030' },
  dangerRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  dangerTitle: { color: '#C04040', fontSize: 14, fontWeight: '700' },
  dangerSub: { color: '#7A6A62', fontSize: 13, marginTop: 2 },
  dangerBtn: { backgroundColor: '#FDF5F5', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, borderWidth: 1, borderColor: '#C04040' },
  ...
  button: { backgroundColor: '#C96A50', borderRadius: 10, paddingVertical: 16, alignItems: 'center', width: '100%' },
