...
// the first user's actual stack — Dr. Worms Einnahme-Tagesplan (05.03.2026, 62 kg)
// followed by Sabine Hoffmann (Heilpraktikerin) addendum (02.06.2026).
// Order matches the Worms sheet top-to-bottom, Hoffmann appended.
// Slot mapping from T0 (morning anchor):
//   MO=0  VM=180  MI=300  NM=480  AB=660  NA=900
const SUPPLEMENTS = [
  { id: 'vit_adek',        name: 'Vitamin ADEK 5000 IE',       form: 'capsule', category: 'core',     notes: 'Fat-soluble cluster (A+D+E+K). 8 caps with late-morning meal. Stop during antibiotics. Dosed for 62 kg.' },
  ...
  // Sabine Hoffmann addendum 02.06.2026
...
// Worms sheet timing is literal; soft conflicts handled via supplement_conflicts.
...
// Soft conflicts — UI warnings, not auto-shifts. Worms timing is honored as-is.
const CONFLICTS = [
  { supplement_id_a: 'zinc',     supplement_id_b: 'magnesium', min_separation_minutes: 60,  notes: 'Zinc and magnesium compete for absorption — ideally space, but Worms plan schedules both at MO.' },
...
export async function seedDb(): Promise<void> {
  ...
  if (!restrictedSeeded || restrictedSeeded.count === 0) {
    const DIETARY_RESTRICTIONS: {
      ...
      { ingredient: 'dairy', aliases: 'Milch, Laktose, Magermilchpulver, Molke', severity: 'forbidden', notes: 'All dairy products are forbidden under Dr. Worms protocol' },
